...
for (table in dbListTables(con_origin)) {
    ...
    for (i in (1:ncol(official_panel))) {
        for (j in (1:ncol(ws_panel))) {
            ...
            kpss_test_diff <- ur.kpss(diff, type = "mu", lags="long")
            # Results in one object
            results_inner <- data.frame(
                kpss_stat = kpss_test_diff@teststat,
                kpss_crit = kpss_test_diff@cval[2]
            )
            colnames(results_inner) <- c("kpss_stat", "kpss_crit")
    ...
    simple_quant <- quantile(results_inner_df$kpss_stat, c(0.025, 0.975), type = 8)
    out <- list(
        t=as.matrix(results_inner_df$kpss_stat),
        t0=kpss_test_diff@teststat,
        R=10000
    )
    class(out) <- "boot"
